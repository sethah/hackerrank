...
class KeyHeap[T: ClassTag](implicit ord: Ordering[T]) extends PriorityQueue[T] {
  ...
  private val keys = scala.collection.mutable.HashMap[T, Int]()
  private var back = 1
  private val front = 1
  ...
  def decreaseValue(value: T, newValue: T): Unit = {
    // figure out where this value is in the array
    // decrease it, then bubble up
    val valueIdx = keys(value)
    data(valueIdx) = newValue
    bubbleUp(valueIdx)
  }
